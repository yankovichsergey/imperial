export const environment = {
    production: true,
    mock: true,
    apiWebPrefics: 'api/',
    apiWebHost: 'https://localhost:44388/',
    userProfileKeyStorage: '__userProfileKeyStorageImperial_mock__',
    auth: {
        paccesToken: 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiIxOTIuMTY4LjEuMjAxIiwiZXh0IjoxNTczMTQ5Mzk0LCJpZCI6MzM3fQ.JvXvIczx1qupUnAtGpoWKvQjXutHDBINLB228dQpq_Y',
        superUserPassword: 'admin'
    },
    authority: 'https://login.microsoftonline.com/common/',
    apiAccessUrl: 'api://58f8afe9-ab33-4302-8b19-cc7489edc841/api-access',
    clientId: '58f8afe9-ab33-4302-8b19-cc7489edc841',
    redirectUri: 'https://yankovichsergey.github.io/imperialTest/',
    postLogoutRedirectUri: 'https://yankovichsergey.github.io/imperialTest/',
    dropdownSheetId: '840667FF0B20A850!298',
    resultSheetId: '840667FF0B20A850!139',
    folderId: '840667FF0B20A850!103',
    driveId: '840667ff0b20a850'
};
